// ./meter_gen.js 3 '30/01/2015' '31/01/2015' 10 -separateFile
var moment = require('moment');
var fs = require('fs');
var locations = require('./locations.json');
var plotly = require('plotly')('dienhoa','yhfhzm1iyd');
...
for (var d = moment(FROM, 'DD/MM/YYYY'); d <= moment(TO, 'DD/MM/YYYY'); d.add(MINUTES_INTERVAL, 'minutes')) {
  for (var i = 0; i < meters.length; i++) {
    ...
    if ((d.format('MM')<11)&&(d.format('MM')>03)) { // Hot season
      if ((d.format('dddd') != "Sunday")&&(d.format('dddd') != "Saturday")) { // Workind day
        if ( (d.format('HH:mm')<='06:00') && (d.format('HH:mm')>='00:00') )  meters[i].conso = rand.rnorm(200, 20)/(60/MINUTES_INTERVAL); 
        ...
        if ( (d.format('HH:mm')<='17:00') && (d.format('HH:mm')>='09:00') )  meter.conso = rand.rnorm(600, 80)/(60/MINUTES_INTERVAL); 
  ...
  if ((d.format('HH:mm') <= '06:00')||(d.format('HH:mm')>= '22:00')){
    ...
    line.push(0.001*meter.highcost);
