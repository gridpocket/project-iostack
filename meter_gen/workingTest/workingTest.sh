...
node ../js/meter_gen.js -metersType "elec" -out './out/out-elec.csv'

echo '\nTEST GAS'
# Generating 100 meters, with data for 5 monthes, one data by meter each 60minutes, all meters from gas heater houses
node ../js/meter_gen.js  -metersType "gas" -out './out/out-gas.csv'
...
node ../js/meter_gen.js -out './out/out-mixed.csv'
...
node ../js/meter_gen.js -temp -out './out/temp.csv'

echo '\nTEST LOCATION'
# Generating 100 meters, with data for 5 monthes, one data by meter each 60minutes, all meters from random heated houses, printing city name, region number and meter location
node ../js/meter_gen.js -location -out './out/location.csv'
...
node ../js/meter_gen.js -lastID 10 -out './out/lastID.csv'
...
node ../js/meter_gen.js -startID 90 -out './out/startID.csv'
...
node ../js/meter_gen.js -startID 45 -lastID 55 -out './out/start&last.csv'
...
node ../js/meter_gen.js -out './out/sepFilesDay/%y-%M-%D_%h-%m.csv'
...
node ../js/meter_gen.js -out './out/sepFilesMonth/%Y/%M.csv'
...
node ../js/meter_gen.js -startID 45 -lastID 55 -location -temp -out './out/sepFiles_start_last/%Y-%M.csv'
...
node ../js/meter_gen.js -maxFileSize 1M -out './out/maxFileSize10k/%N.csv'
...
node ../js/meter_gen.js -maxFileSize 1M -startID 45 -lastID 55 -location -temp -out './out/maxFileSize_start_last/%N.csv'
...
node ../js/meter_gen.js -maxFileSize 100k -out './out/sepFiles_maxFileSize/%Y-%M_%N.csv'