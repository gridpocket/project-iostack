...
echo '1 Day, 1/30min, 100 users, mixed' # 4.800 data, ~700Ko
ts=$(date +%s%N)
...
tmps=$((($(date +%s%N) - $ts)/1000000))
size=$(stat -c %s './perfTest/07d-1ku.csv')
echo "Time: $tmps, Size: $size"
...
tmps=$((($(date +%s%N) - $ts)/1000000))
size=$(stat -c %s './perfTest/31d-1ku.csv')
rm './perfTest/31d-1ku.csv' &
echo "Time: $tmps, Size: $size"

echo '1 Year, 1/30min, 1000 users, mixed' # 17.520.000 data
ts=$(date +%s%N)
node meter_gen.js 1000 '2016/01/01' '2017/01/01' 30 mixed -location -temp -out './perfTest/12m-1ku.csv'
tmps=$((($(date +%s%N) - $ts)/1000000))
size=$(stat -c %s './perfTest/12m-1ku.csv')
rm './perfTest/12m-1ku.csv' &
echo "Time: $tmps, Size: $size"


echo
echo '1 Day, 1/30min, 10000 users, mixed' # 480.000 data, ~65Mo
ts=$(date +%s%N)
